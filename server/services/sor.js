...
export const getHourlyData = async date => {
  ...
  try {
    ...
    // Zapytanie dla danych godzinowych wybranego dnia --> dla pojedynczych rekordow per godzina
    const queryPatientsPerHour = `
            WITH RECURSIVE godziny AS (
                SELECT 0 AS godzina
                UNION ALL
                SELECT godzina + 1 FROM godziny WHERE godzina < 23
            )
            SELECT
                COUNT(id) AS liczba_pacjentow
            FROM 
                godziny
            LEFT JOIN 
                pacjenci
            ON 
                HOUR(data_przyjecia) = godzina 
                AND DATE(data_przyjecia) = DATE( ? )
            GROUP BY 
                godzina
            ORDER BY 
                godzina ASC;
    ...
    const [rowsPatientsPerHour] = await connection.query(queryPatientsPerHour, [startDate]);
    const [rowsPrevDay] = await connection.query(queryPrevDay, [prevDayLastHour, startDate]);

    return {
      currentDayData: rowsCurrentDay,
      patientsPerHourData: rowsPatientsPerHour,
